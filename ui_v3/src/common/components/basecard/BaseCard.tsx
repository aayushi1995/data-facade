...
export const BaseCard = ({
    ...
    return <Card sx={{borderRadius: 1, background, height, p: 1, width, position: 'relative', boxShadow: boxShadow || theme.shadows[23]}}>
        <Stack direction="column" gap={1} justifyContent={'space-between'} height={'100%'}>
                {children}
                {PrimaryActionIconButtons}
            </Stack>
            <Stack direction="column" sx={{position: 'absolute', right: 10, top: 10}}>
                {ActionIconButtons}
            </Stack>
    </Card>;
};
