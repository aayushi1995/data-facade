...
const RenderSingleValueChart = (props) => {
    ...
    if (completedActionExecutions.length > 1) {
        completedActionExecutions.forEach((execution) => {
            const output = parseInt(JSON.parse(execution.Output).Value)
        ...
        const output = parseInt(JSON.parse(completedActionExecutions[0].Output).Value)
...
const ActionInstanceCard = (props) => {
            ...
            <Grid container spacing={2} style={{display: 'flex', alignContent: 'center'}}>
                <Grid item xs={12}>
                    <Grid container spacing={2} style={{
                        ...
                        <Grid item xs={1} style={{display: 'flex', alignContent: 'center', alignSelf: 'center'}}>
...
const QuickStatsNewTemp = (props) => {
    ...
    const actionDefinitionConfig = {
        ...
        body: JSON.stringify({
            ...
            "actionProperties": {
                "filter": {
                    "UniqueName": props.columnUniqueName
                },
    ...
    } = useQuery(`FetchActionDefinitionInQuickStatsColumn${props.colummUniqueName}`, () =>
...
const QuickStatsNewTempWrapper = (props) => {
    ...
    React.useEffect(() => {
        const toScroll = document.getElementById('columnDetailsQuickStats-container')
    ...
    return (
        <>
            ...
            <div id="columnDetailsQuickStats-container">
                <Grid container spacing={0}>
                    {
                        imgData.images.map((val, index) => (
                            <Grid item xs={6} style={{padding: 20}}
                                  key={`ActionInstanceCardColumn${val.instance.ActionInstance.Id}${index}`}>
